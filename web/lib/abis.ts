...
export const vaultAbi = [
  ...
  // V2 zap redeem: same proportional unwind as redeem(), then swaps up to
  // `otherToSwap` of the non-asset token into the asset via the V3 zap
  // adapter so the user always receives `assets` denominated in `asset()`.
  // Required whenever the LP position is IN_RANGE — plain redeem reverts
  // with InsufficientAssetUseZap when the unwound non-asset leg is non-zero.
  {
    type: 'function',
    name: 'redeemWithZap',
