...
// Vendored verbatim from Uniswap v4-core test utilities (commit pinned via
// `lib/v4-core/test/utils/LiquidityAmounts.sol`). Imports are rewritten to
// point at the same canonical libraries through the public package alias so
// production contracts no longer pull from the v4-core test path.
//
// Behaviour MUST remain bit-identical to the upstream reference. Do not
// modify the math; if upstream changes, re-vendor the entire file.
